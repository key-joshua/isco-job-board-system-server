...
const isAccountVerified = async (req, res, next) => {
  try {
    ...
    const user = await authRepository.findUserByAttributes({ whereKey: 'id', whereValue: verifiedToken.id });
...
const isSessionExist = async (req, res, next) => {
  try {
    ...
    if (!session) {
      responseUtils.handleError(StatusCodes.UNAUTHORIZED, 'Access token is expired or invalid. Either Email verification link expired, invalid or logged out.');
      return responseUtils.response(res);
    }
