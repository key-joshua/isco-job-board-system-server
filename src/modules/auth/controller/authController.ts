...
const signup = async (req, res) => {
  try {
    ...
    const accessToken = generateAccessToken(user.id, process.env.JWT_SECRET);
    const session = await authRepository.createSession({ user_id: user.id, device_id: req.headers['user-device'], access_token: accessToken, refresh_token: refreshToken });
...
const signin = async (req, res) => {
  try {
    ...
    const refreshToken = generateRandomString();
    const accessToken = generateAccessToken(req.user.id.toString(), process.env.JWT_SECRET);
    const session = await authRepository.createSession({ user_id: req.user.id, device_id: req.deviceId, access_token: accessToken, refresh_token: refreshToken });
...
const verifyEmail = async (req, res) => {
  try {
    const user = await authRepository.updateUserByAttributes({ updatedKey: 'is_verified', updatedValue: true, whereKey: 'id', whereValue: req.user.id });
...
const signout = async (req, res) => {
  try {
    await authRepository.destroySessionByAttribute({ updatedKey: 'user_id', updatedValue: req.user.id, whereKey: 'access_token', whereValue: req.session.access_token });
...
const sendVerificationLink = async (req, res) => {
  try {
    ...
    const refreshToken = generateRandomString();
    const accessToken = generateAccessToken(user.id, process.env.JWT_SECRET);
    await authRepository.createSession({ user_id: user.id, device_id: req.headers['user-device'], access_token: accessToken, refresh_token: refreshToken });
...
const resetPassword = async (req, res) => {
  try {
    const user = await authRepository.updateUserByAttributes({ updatedKey: 'password', updatedValue: hashPassword(req.body.password), whereKey: 'id', whereValue: req.user.id });
    await authRepository.destroySessionByAttribute({ updatedKey: 'user_id', updatedValue: req.user.id, whereKey: 'access_token', whereValue: req.session.access_token });
