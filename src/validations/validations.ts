...
const signupSchema = Joi.object({
  profile_picture: Joi.string().messages({
      'string.base': 'profile_picture should be a type of string',
  ...
  status: Joi.string().valid('ADMIN', 'APPLICANT').optional().messages({
    'string.base': 'status must be a string',
    'any.only': 'status must be either ADMIN or APPLICANT',
  }),
});
